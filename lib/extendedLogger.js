...
var reqIdMiddleware = require('./reqIdMiddleware');
...
function _getReqId() {
  return '[' + reqIdMiddleware.getRequestId() + ']';
}
...
module.exports = function (moduleId, displayConf) {
  ...
  //Remove the winston Logger module from cache, so it can be instantiated
  //and overload with a new specific configuration
  ...
  //Extends the log prototype function, for new instances of winston
  ...
  //Adds a facade function to the default Winston logger, which is already instantiated
  ...
  extendedInterface.reqIdMiddleware = reqIdMiddleware;
